import React, { createContext, ReactNode, useState, useEffect } from "react";

interface ChatContextProps {
  username: string
  image: string
  friends: any
}

interface ChatProviderProps {
  children: ReactNode;
}

const ChatContext = createContext<ChatContextProps | undefined>(undefined);

export var tk: string | null;


function ChatProvider({ children }: ChatProviderProps) {

    const [username, setUsername] = useState("")
    const [image, setUserImage] = useState("data:image/png;base64,")
    const [friends, setFriends] = useState([])
    const [userId, setUserId] = useState("")

    useEffect( () => {

        const urlParams = new URLSearchParams(window.location.search);
        tk = urlParams.get('token');

        console.log("Frontend: token", tk);

        fetch('http://localhost:3000/user/me', {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${tk}`,
            'Content-Type`': 'application/json',
        }
        })
        .then(async (response) => await response.json())
        .then((data) => {
            setUsername(data.username)
            setUserImage(data.image)
            setUserId(data.id)
        })
        .catch((error) => console.log(error))
                
        fetch(`http://localhost:3000/user/friends`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${tk}`,
            'Content-Type': 'application/json',
          }
        })
          .then(async (response) => {
            if (!response.ok) {
              throw new Error(`HTTP error! Status: ${response.status}`);
            }
            const data = await response.text(); // or response.json() if the response is JSON
            return data ? JSON.parse(data) : null; // Check if data is not empty before parsing
          })
          .then((data) => {
            if (data) {
              console.log("Friends received ", data);
              setFriends(data.friends);
            } else {
              console.log("No data received");
            }
          })
          .catch((error) => console.error('Fetch error:', error));



    }, []);
  

  const contextValue: ChatContextProps = {
    username: username,
    image: image,
    friends: friends
  };

  return (
    <ChatContext.Provider value={contextValue}>
      {children}
    </ChatContext.Provider>
  );
}

export { ChatContext, ChatProvider };
